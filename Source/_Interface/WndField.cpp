	...
	for( int i=2; i<MAX_HUMAN_PARTS; i++ )
	{
		...
		if( pItemBase && pItemBase->GetTexture() )
		{
			...
			if (pItemElem->GetProp()->dwPackMax > 1)
			{
				...
				m_p2DRender->TextOut( x + 41 - size.cx, y + 41 - size.cy, szTemp, 0xFF85FF8A );
...
#if __VER >= 19
					|| pItemMaterialElem->GetProp()->dwItemKind3 == IK3_SMELT_ULTIMATE_MATERIAL
...
#endif // __ULTIMATE			
#if __VER >= 19
					|| pItemMaterialElem->GetProp()->dwItemKind3 == IK3_SMELT_ACCESSORY_MATERIAL
#endif
					)
...
void CWndCharInfo::OnDraw(C2DRender* p2DRender)
{
...
#ifdef __STATISTICS
	y = 10;
	p2DRender->TextOut(86, y, prj.GetText((TID_APP_CHARACTER_BASE)), dwColor);
#else
	y = 13;
	...
	y = 10 + nyAdd3;
#if __VER >= 19
	dwColor = D3DCOLOR_ARGB(255, 246, 204, 77);
#else
...
#ifdef __STATISTICS
#if __VER >= 19
	p2DRender->TextOut(86, 112, prj.GetText((TID_APP_CHARACTER_DETAIL)), D3DCOLOR_ARGB(255, 255, 255, 255));
#else
	p2DRender->TextOut(56, 109, prj.GetText((TID_APP_CHARACTER_DETAIL)), D3DCOLOR_ARGB(255, 0, 0, 0));
#endif
#else
#if __VER >= 19
	p2DRender->TextOut(60, 113, prj.GetText((TID_APP_CHARACTER_DETAIL)), D3DCOLOR_ARGB(255, 255, 255, 255));
#else
	...
	{
#if __VER >= 19
		dwColor = D3DCOLOR_ARGB(255, 74, 123, 197);
#else
	...
	p2DRender->TextOut(95, y, prj.GetText(TID_CRT_RATE), dwColor); y += nNext;
	p2DRender->TextOut(7, y, prj.GetText(TID_TOOLTIP_DEFENCE), dwColor);
#ifdef __STATISTICS
	p2DRender->TextOut(95, y, prj.GetText(TID_GAME_CHARACTER_15), dwColor); y += nNext;
	p2DRender->TextOut(7, y, prj.GetText(TID_DODGE), dwColor);
	p2DRender->TextOut(95, y, prj.GetText(TID_HIT_RATE), dwColor); y += nNext;
	p2DRender->TextOut(7, y, prj.GetText(TID_GAME_CHARACTER_14), dwColor);
	p2DRender->TextOut(95, y, prj.GetText(TID_M_BLOCK), dwColor); y += nNext;
	p2DRender->TextOut(7, y, prj.GetText(TID_SPD_RUN), dwColor);
	p2DRender->TextOut(95, y, prj.GetText(TID_R_BLOCK), dwColor); y += nNext;
	p2DRender->TextOut(7, y, prj.GetText(TID_SPL_SPEED), dwColor);
	p2DRender->TextOut(95, y, prj.GetText(TID_PVP_DAMAGE), dwColor); y += nNext;
	y += 10;
#else
	p2DRender->TextOut(95, y, prj.GetText(TID_SPD_RUN), dwColor); y += nNext;
	y += 12;
#endif
...
void CWndCharInfo::OnInitialUpdate()
{
...
#ifdef __STATISTICS
#if __VER >= 19
	SetTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, _T("WndNewCharacterBlack.tga")), TRUE);
#endif
#endif
	...
	m_wndStrPlus.SetTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, _T("ButtVScrUp.tga")), TRUE);
	m_wndStrMinus.SetTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, _T("ButtVScrDown.tga")), TRUE);
	m_wndStaPlus.SetTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, _T("ButtVScrUp.tga")), TRUE);
	m_wndStaMinus.SetTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, _T("ButtVScrDown.tga")), TRUE);
	m_wndDexPlus.SetTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, _T("ButtVScrUp.tga")), TRUE);
	m_wndDexMinus.SetTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, _T("ButtVScrDown.tga")), TRUE);
	m_wndIntPlus.SetTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, _T("ButtVScrUp.tga")), TRUE);
	m_wndIntMinus.SetTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, _T("ButtVScrDown.tga")), TRUE);
...
#if __VER >= 19
	int nPos[3]; nPos[0] = 68; nPos[1] = 142; nPos[2] = posY + 5;
	CSize size = CSize(75, 25);
	m_wndApply.Create(prj.GetText(TID_MMI_UIBUTTON_BUTTON02), 0, CRect(nPos[0] - (size.cx / 2), nPos[2], (nPos[0] - (size.cx / 2)) + size.cx, nPos[2] + size.cy), this, 112);
	m_wndReset.Create(prj.GetText(TID_MMI_UIBUTTON_BUTTON03), 0, CRect(nPos[1] - (size.cx / 2), nPos[2], (nPos[1] - (size.cx / 2)) + size.cx, nPos[2] + size.cy), this, 113);
#endif

	if (::GetLanguage() == LANG_FRE)
	{
#if __VER >= 19
		m_wndApply.SetTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, _T("ButtNormal00.tga")), TRUE);
		m_wndReset.SetTexture(m_pApp->m_pd3dDevice, MakePath(DIR_THEME, _T("ButtNormal00.tga")), TRUE);
#endif
	}
	else
	{
...
void CWndCharInfo::RenderATK(C2DRender* p2DRender, int x, int y)
{
	...
	if ((m_nStrCount != 0 || m_nDexCount != 0 || m_nIntCount != 0) && (nTemp1 != nMin || nTemp2 != nMax))
	{
		...
		{
#if __VER >= 19
			dwColor = D3DCOLOR_ARGB(255, 121, 80, 255);
#else //__VER >= 19
			dwColor = D3DCOLOR_ARGB(255, 255, 0, 0);
#endif //__VER >= 19
		}
...
void CWndCharacterDetail::OnDraw(C2DRender* p2DRender)
{
	...
	for( int iC = 0 ; iC < 5 ; ++iC )
	{
		if( rectHittest[iC].PtInRect( ptMouse ) )
		{
			...
			CEditString strEdit;
			CString szString;
			DWORD dwColorName = D3DCOLOR_ARGB( 255, 246, 204, 77 );
...
void CWndCharacterDetail::OnInitialUpdate()
{
	...
	m_wndStr.SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, _T( "ButtVScrUp.tga" ) ), TRUE );
	m_wndSta.SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, _T( "ButtVScrUp.tga" ) ), TRUE );
	m_wndDex.SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, _T( "ButtVScrUp.tga" ) ), TRUE );
	m_wndInt.SetTexture( m_pApp->m_pd3dDevice, MakePath( DIR_THEME, _T( "ButtVScrUp.tga" ) ), TRUE );
...
BOOL CWndNavigator::OnEraseBkgnd(C2DRender* p2DRender)
{
	...
	pd3dDevice->SetRenderTarget( 0, pDstSurface[0] );  
	pd3dDevice->Clear( 0, NULL, D3DCLEAR_ZBUFFER | D3DCLEAR_TARGET, 0x00000000, 1.0f, 0 );
...
//						pLand->m_texMiniMap.m_size = m_size;
#ifdef __NAVIGATOR_V19
						if( pWorld->m_bIsIndoor == FALSE && !pWorld->IsWorldInstanceDungeon() )
						{
#ifdef __MAP_TORCH_SYSTEM
							if( dwWorldID >= WI_WORLD_KEBARAS && dwWorldID <= WI_WORLD_DARKRARTESIA 
							|| dwWorldID >= WI_WORLD_HEAVEN01 && dwWorldID <= WI_WORLD_HEAVEN05
							|| dwWorldID >= WI_INSTANCE_OMINOUS && dwWorldID <= WI_INSTANCE_SANPRES_1
							|| dwWorldID >= WI_DUNGEON_FL_MAS && dwWorldID <= WI_DUNGEON_DA_DK
							|| dwWorldID >= WI_DUNGEON_SA_TA && dwWorldID <= WI_DUNGEON_SA_TA2
							|| dwWorldID == WI_DUNGEON_VOLCANE
							|| dwWorldID == WI_DUNGEON_VOLCANERED
							|| dwWorldID == WI_DUNGEON_VOLCANEYELLOW
							|| dwWorldID == WI_INSTANCE_CONTAMINTRAILS
							|| dwWorldID == WI_INSTANCE_CONTAMINTRAILS
							|| dwWorldID == WI_WORLD_MINE
							)
							{
								...
								{
									p2DRender->RenderForMask_Main( &pLand->m_texMiniMap, point );
								}
								else
									p2DRender->RenderForMask_Main( &m_texDunFog, CPoint( 2,2 ) );
							}
							else
#endif // __MAP_TORCH_SYSTEM
							{
								p2DRender->RenderForMask_Main( &pLand->m_texMiniMap, point );
							}
						}
...
#ifdef __NAVIGATOR_V19
							p2DRender->RenderForMask_Main( &m_GuildCombatTextureMask, point );
#else // __NAVIGATOR_V19
...
#ifdef __NAVIGATOR_V19
	p2DRender->RenderForMask_BG( &m_ALPHACIRCLE, CPoint(0,0), 255 );
	pd3dDevice->SetRenderTarget( 0, pOldSurface);
	pOldSurface->Release();

	pTextureMap.m_pTexture->GetSurfaceLevel( 0, &pSurfaceDst );
	pd3dDevice->StretchRect( pDstSurface[0], NULL, pSurfaceDst, NULL, D3DTEXF_NONE);

	pTextureMap.Render( p2DRender, CPoint(0,0), 250 );
	...
	if( pWorld->m_bIsIndoor == FALSE && !pWorld->IsWorldInstanceDungeon() )
	{
		...
		)
		{
			bShow = 
			(
				g_pPlayer->HasBuffByIk3( IK3_MAPOPEN_DUNGEON )
				&& dwWorldID != WI_WORLD_MINE
			);
		}
		else
...
//				BOOL bPartyMap = FALSE;
				if( pObj != g_pPlayer )
				{
					...
					if( rectHittest.PtInRect( ptMouse ) )
					{
						...
						ClientToScreen( &rectHittest );
#ifdef __NAVIGATOR_V19
						float fDistance = (float)sqrt(pow((rect.Width()/2)-point.x,2) + pow((rect.Height()/2)-point.y,2));
						if( fDistance < 65 )
#endif // __NAVIGATOR_V19
						{
					...
					if( nCount < 1000 )
					{
						...
						else
						{
							...
							{
#ifdef __NAVIGATOR_V19
								CRect rDistance = CRect( rect.left -8,  rect.top -8, rect.right - 8 , rect.bottom - 8 );
								if( rDistance.PtInRect( point ) )
#endif // __NAVIGATOR_V19
								{
...
#ifdef __NAVIGATOR_V19
	pTopTexture.m_size = CSize(rect.Width(), rect.Height());
	pTopTexture.Render(p2DRender, CPoint(0, 0), 255);

/*
	CPoint pTextPoint;
	CSize sizeTitle = p2DRender->m_pFont->GetTextExtent(m_strTitle);

	pTextPoint = CPoint((rect.Width() / 2) - (sizeTitle.cx / 2), -10);
	CD3DFont* pOldFont = p2DRender->GetFont();
	p2DRender->SetFont(CWndBase::m_Theme.m_pFontWndTitle);
	p2DRender->TextOut( pTextPoint.x - 4, pTextPoint.y - 8 , m_strTitle, 0xffffffff );
	p2DRender->SetFont( pOldFont );
	*/
#endif // __NAVIGATOR_V19
...
HRESULT CWndNavigator::DeleteDeviceObjects()
{
	...
	pTopTexture.DeleteDeviceObjects();
#endif // __NAVIGATOR_V19
#endif //__YDEBUG
...
HRESULT CWndNavigator::RestoreDeviceObjects()
{
	...
	pTopTexture.SetInvalidate(m_pApp->m_pd3dDevice);
#endif // __NAVIGATOR_V19
...
#if __VER >= 19
#ifdef __NAVIGATOR_V19
	m_wndPlace.Create  ( "", 0, CRect( rectClient.left,   1, rectClient.left + 18,   1 + 18 ), this, 100000 ); // <
...
#else // __NAVIGATOR_V19
	m_wndPlace.Create("", 0, CRect(rectClient.left, 1, rectClient.left + 16, 1 + 16), this, 100000);
	//	m_wndMover.Create  ( "", 0, CRect( rectClient.left,  17, rectClient.left + 16,  17 + 16 ), this, 100001 );
	//	m_wndMover.Create  ( "", 0, CRect( rectClient.left,  36, rectClient.left + 16,  36 + 16 ), this, 102 );
...
#ifdef __NAVIGATOR_V19
	D3DDEVICE->CreateRenderTarget( 180, 176, D3DFMT_A8R8G8B8, g_Neuz.m_d3dSettings.MultisampleType(), g_Neuz.m_d3dSettings.MultisampleQuality(),FALSE, &pDstSurface[0], NULL);
	pTextureMap.CreateTexture( g_Neuz.m_pd3dDevice, 180, 176, 0, D3DUSAGE_RENDERTARGET, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT );

	m_texDunFog.LoadTexture(g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "map3_1.tga"), 0, 1);
	m_ALPHACIRCLE.LoadTexture(g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "alphacircle.tga"), 0, 1);;
	pTopTexture.LoadTexture(g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, "map.tga"), 0, 1);
#else // __NAVIGATOR_V19
...
BOOL CWndNavigator::Initialize(CWndBase* pWndParent,DWORD dwWndId)
{
	CRect rectWindow = m_pWndRoot->GetWindowRect();
#ifdef __NAVIGATOR_V19
	CRect rect( 0, 0, 200, 200 ); // 1024 768
#else // __NAVIGATOR_V19
...
void CWndStatus::MakeGaugeVertex()
{
	...
	//lse m_bVBFPGauge = TRUE;
		...
		if( m_nPXPWidth != GuildHouse->m_nExtraExp )
		{
			...
			ClientToScreen( rectTemp );
#if __VER >= 19
			m_bVBAEXPGauge = m_pTheme->MakeGaugeVertex( m_pApp->m_pd3dDevice, &rectTemp, 0xffffffff, m_pVBAEXPGauge, &m_AEXPGaugeTexture );
#else // __VER >= 19
...
//	prj.m_modelMng.LoadMotion(m_pModel,g_pPlayer->m_dwType,g_pPlayer->m_dwIndex,0);
	...
	m_EXPGaugeTexture.LoadTexture(m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "Targetgauge02.tga"), 0xffff00ff, TRUE);
	m_AEXPGaugeTexture.LoadTexture(m_pApp->m_pd3dDevice, MakePath( DIR_THEME, "Targetgauge02.tga"), 0xffff00ff, TRUE);
#endif // __VER >= 19
	//m_texGauEmptySmall  
	//m_texGauFillNormal  
	//m_texGauFillSmall   
...
#if __VER >= 19
				|| pItemProp->dwItemKind3 == IK3_SMELT_ULTIMATE_MATERIAL
...
#if __VER >= 19
				|| pItemProp->dwItemKind3 == IK3_SMELT_ACCESSORY_MATERIAL
#endif
				)
		{
	...
	if( m_nItem != 0xff )
	{
		...
		if( pItemElem )
		{
			...
			if( pItemElem->GetProp()->dwPackMax > 1 && pItemElem->GetExtra() > 0 )
			{
				...
				p2DRender->TextOut( pCustom->rect.right-13,  pCustom->rect.bottom-13 , szTemp, 0xFF000000 );
				p2DRender->TextOut( pCustom->rect.right-14,  pCustom->rect.bottom-14 , szTemp, 0xFF85FF8A );
#else // __VER >= 19
	...
	for( int i=nBase; i<nBase + MAX_MAIL_LIST_PER_PAGE; ++i )
	{
		...
		if( mailbox[i]->m_pItemElem && mailbox[i]->m_pItemElem->m_pTexture )
		{
			...
			if( mailbox[i]->m_pItemElem->GetProp()->dwPackMax > 1 )
			{
				...
				p2DRender->TextOut( sx+27,  sy+20 , szTemp, 0xFF000000 );
				p2DRender->TextOut( sx+27,  sy+20 , szTemp, 0xFF85FF8A );
#else // __VER >= 19
...
//		if( GetFocusWnd() == this )
		{
			...
			if( PtInRect( rect, point ) )
			{
				...
				strDateTime.Format(prj.GetText(TID_GAME_MAIL_RECEIVE_DATE), dt.Format("%d/%m/%Y"));
...
void CWndGuildCombatBoard::PaintFrame( C2DRender* p2DRender )
{
	...
	if( IsWndStyle( WBS_CAPTION ) )	
	{
		...
		CSize sizeTitle = p2DRender->m_pFont->GetTextExtent( m_strTitle );
		CRect rect = GetWindowRect();
		CPoint newpoint = (rect.Width() - sizeTitle.cx) / 2;
...
void CWndBuffStatus::RenderOptBuffTime(C2DRender *p2DRender, CPoint& point, CTimeSpan &ct, DWORD dwColor )
{
	if(g_Option.m_bVisibleBuffTimeRender)
	{
		...
		if( seconds >= 3600 ){
			...
			p2DRender->TextOut(point.x+2, point.y+20, str+"h", dwColor, 0xFF000000);
		}
		else if( seconds >= 60 ){
			seconds = (int)(ct.GetTotalMinutes());
			str.Format( "%d" , seconds );
			p2DRender->TextOut(point.x+2, point.y+20, str+"m", dwColor, 0xFF000000);
		}
		else {
			...
			str.Format( "%d" , seconds );
			p2DRender->TextOut(point.x+2, point.y+20, str+"s", dwColor, 0xFF000000);
		}
		p2DRender->SetFont( pOldFont );
	}
}
...
{ 
	...
	if(::GetLanguage() == LANG_FRE)
#if __VER >= 19
		pButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, _T("ButtNormal00.tga")), TRUE);
#else // __VER >= 19
		...
		case CUltimateWeapon::ULTIMATE_SUCCESS:
			{
				...
				g_WndMng.PutString(message);
				m_pItemElem = NULL;
			}
			break;
		case CUltimateWeapon::ULTIMATE_FAILED:
		case CUltimateWeapon::ULTIMATE_CANCEL:
			g_WndMng.PutString(prj.GetText(TID_GAME_EXTRACTION_FAILED), NULL, prj.GetTextColor(TID_GAME_EXTRACTION_FAILED));
			Destroy();
			break;
		case CUltimateWeapon::ULTIMATE_INVENTORY:
			g_WndMng.PutString(prj.GetText(TID_GAME_EXTRACTION_ERROR), NULL, prj.GetTextColor(TID_GAME_EXTRACTION_ERROR));
...
#if __VER >= 19
			pWndStatic->m_dwColor = 0xffffffff;
#else // __VER >= 19
...
BOOL CWndSmeltSafety::Process()
{
	...
	if(m_bStart != FALSE && m_pItemElem != NULL)
	{
		...
		if(m_dwEnchantTimeEnd < g_tmCurrent)
		{
			...
			m_dwEnchantTimeEnd = g_tmCurrent + SEC(1);
		}
	}
	else
	{
...
//		m_pTheme->RenderGauge( p2DRender, &rectTemp, 0x8437dc37 , m_pVBEXPGauge, &m_texGauFillSmall );
...

	{
		...
		p2DRender->TextOut( x + 20, lpExp->rect.top - 0, cbufExp, dwColor, 0xff000000 );
	}	
...
void CWndLinkedItem::InsertItem(CItemElem* pItemElem)
{
	SAFE_DELETE(m_pItemElem);
	m_pItemElem = pItemElem;

	MakeToolTipText(m_strItem);
	if (pItemElem->GetProp()->IsBaruna())
		m_pTex = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, "icon_Baruna01.dds"), 0xffff00ff);
	else if (pItemElem->GetProp()->IsUltimate())
		m_pTex = CWndBase::m_textureMng.AddTexture(g_Neuz.m_pd3dDevice, MakePath(DIR_ICON, "Icon_Ultimate.dds"), 0xffff00ff);
	else
		m_pTex = nullptr;
}
void CWndLinkedItem::OnSize(UINT nType, int cx, int cy)
{
	CWndNeuz::OnSize(nType, cx, cy);
}
BOOL CWndLinkedItem::Initialize(CWndBase* pWndParent, DWORD nType)
{
	return CWndNeuz::InitDialog(g_Neuz.GetSafeHwnd(), APP_TOOLTIP_LINKEDITEM, 0, CPoint(0, 0), pWndParent);
}

BOOL CWndLinkedItem::OnChildNotify(UINT message, UINT nID, LRESULT* pLResult)
{
	return CWndNeuz::OnChildNotify(message, nID, pLResult);
}
...
void CWndLinkedItem::MakeToolTipText(CEditString &str)
{
	g_WndMng.MakeToolTipText(m_pItemElem, str);
}
void CWndLinkedItem::OnInitialUpdate()
{
	...
	CRect rect = GetClientRect();
	m_strItem.Init(m_pFont, &rect);
	CSize size;
	m_pFont->GetTextExtent_EditString(m_strItem, &size);
	size.cy = m_pFont->GetMaxHeight() * (m_strItem.GetLineCount() + 1);
	m_rectRender = CRect(rect.left, rect.top, rect.left + size.cx + 5, rect.top + size.cy + 22 + 32 + (2 * (size.cy / m_pFont->GetMaxHeight())));
	m_rectRender.bottom += 20;
	m_rectRender.right += 10;
	SetWndRect(m_rectRender);
...
BOOL CWndTreasureBox::OnDropIcon(LPSHORTCUT pShortcut, CPoint point)
{
	...
	if (g_pPlayer->IsUsing(pItemElem))
	{
		g_WndMng.PutString(prj.GetText(TID_GAME_CANNOT_DO_USINGITEM), NULL, prj.GetTextColor(TID_GAME_ERROR_UNDESTRUCTABLE_ITEM));
	}
	else
	{
		if (IsOpenKey(pItemElem))
		{
			if (m_pItemElem == NULL)
			{
				m_pItemElem = pItemElem;
			...
			g_WndMng.PutString(prj.GetText(TID_MMI_TREASURE_ERRORCOMBINE), NULL, prj.GetTextColor(TID_GAME_ERROR_UNDESTRUCTABLE_ITEM));
	}
	if (m_pItemElem != NULL)
	{
...
{
	if ((m_pBoxItem->GetProp()->dwID == II_SYS_SYS_SCR_TREASUREBOXLV15) && (pKey->GetProp()->dwID == II_SYS_SYS_SCR_TREASUREKEYLOW15 || pKey->GetProp()->dwID == II_SYS_SYS_SCR_TREASUREKEYMIDDLE15 || pKey->GetProp()->dwID == II_SYS_SYS_SCR_TREASUREKEYHIGH15))
		return TRUE;
	if ((m_pBoxItem->GetProp()->dwID == II_SYS_SYS_SCR_TREASUREBOXLV60) && (pKey->GetProp()->dwID == II_SYS_SYS_SCR_TREASUREKEYLOW60 || pKey->GetProp()->dwID == II_SYS_SYS_SCR_TREASUREKEYMIDDLE60 || pKey->GetProp()->dwID == II_SYS_SYS_SCR_TREASUREKEYHIGH60))
		return TRUE;
	if ((m_pBoxItem->GetProp()->dwID == II_SYS_SYS_SCR_TREASUREBOXLV90) && (pKey->GetProp()->dwID == II_SYS_SYS_SCR_TREASUREKEYLOW90 || pKey->GetProp()->dwID == II_SYS_SYS_SCR_TREASUREKEYMIDDLE90 || pKey->GetProp()->dwID == II_SYS_SYS_SCR_TREASUREKEYHIGH90))
		return TRUE;
	if ((m_pBoxItem->GetProp()->dwID == II_SYS_SYS_SCR_TREASUREBOXLV120) && (pKey->GetProp()->dwID == II_SYS_SYS_SCR_TREASUREKEYLOW120 || pKey->GetProp()->dwID == II_SYS_SYS_SCR_TREASUREKEYMIDDLE120 || pKey->GetProp()->dwID == II_SYS_SYS_SCR_TREASUREKEYHIGH120))
		return TRUE;
	if ((m_pBoxItem->GetProp()->dwID == II_SYS_SYS_SCR_TREASUREBOXLV150) && (pKey->GetProp()->dwID == II_SYS_SYS_SCR_TREASUREKEYLOW150 || pKey->GetProp()->dwID == II_SYS_SYS_SCR_TREASUREKEYMIDDLE150 || pKey->GetProp()->dwID == II_SYS_SYS_SCR_TREASUREKEYHIGH150))
		return TRUE;
