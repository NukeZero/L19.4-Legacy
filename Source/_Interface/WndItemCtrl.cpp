...
//			case IK2_HATCH:
	...
	if( !g_eLocal.GetState(EVE_SCHOOL) )
	{
		...
		{
#if __VER >= 19
			pItemElem->GetTexture()->m_size = CSize( 32, 32 );

			if( nParent == APP_SHOP_ )
				pItemElem->GetTexture()->Render( p2DRender, CPoint( x + 2, y ), nalpha );
			else
				pItemElem->GetTexture()->Render( p2DRender, CPoint( x + 2, y  + 5), nalpha );
#else // __VER >= 19
			pItemElem->GetTexture()->Render( p2DRender, CPoint( x, y  + 5), nalpha );
#endif // __VER >= 19
		}
		else
		if( bRander == 1 )	// 사용할수 없는것
		{
			...
			if( nParent == APP_SHOP_ )
				pItemElem->GetTexture()->Render2( p2DRender, CPoint( x + 2, y ), D3DCOLOR_ARGB( nalpha, 255, 100, 100 ));
			else
				pItemElem->GetTexture()->Render2( p2DRender, CPoint( x + 2, y  + 5), D3DCOLOR_ARGB( nalpha, 255, 100, 100 ));
#else // __VER >= 19
			pItemElem->GetTexture()->Render2( p2DRender, CPoint( x + 5, y + 5), D3DCOLOR_ARGB( nalpha, 255, 100, 100 ));
#endif // __VER >= 19
		}
	}
	else
...
DWORD CWndItemCtrl::GetColor( CItemElem* pItemElem )
{
	...
	case IK1_ARMOR:
	case IK1_WEAPON:
		if( pItemProp->dwReferStat1 == WEAPON_UNIQUE || pItemProp->dwReferStat1 == WEAPON_ULTIMATE || pItemProp->dwReferStat1 == ARMOR_SET || pItemProp->dwReferStat1 == BARUNA_D )
		{
			if( pItemElem->IsFlag( CItemElem::binds ))
				dwColorbuf = 0xffff0000;
			else
			{
	...
	if( m_pItemContainer ) 
	{
		...
		if( 1 ) //m_dwListCtrlStyle == WLVS_ICON )
		{
			...
			//if( !( nRange - nPage <= 0 ) )
				m_rectClient.right -= 15; // 스크롤 바가 보이면 
#if __VER >= 19
			if( m_dwFrame == APP_SHOP_ )
				m_rectClient.bottom -= 35-32+1;
#endif // __VER >= 19
		}
	}
	if( bOnSize )
...
void CWndItemCtrl::PaintFrame( C2DRender* p2DRender )
{
	...
	*/
	...
	if( 1 ) //m_dwListCtrlStyle == WLVS_ICON )
	{
		...
		nRange = m_pItemContainer->m_dwIndexNum / ( rect.Width() / 35 );
#else // __VER >= 19
		nPage = rect.Height() / 32;
		nRange = m_pItemContainer->m_dwIndexNum / ( rect.Width() / 32 );
#endif // __VER >= 19
	}
	m_wndScrollBar.SetScrollRange( 0, nRange );
	m_wndScrollBar.SetScrollPage( nPage );
