...
//	ON_MSG( PACKETTYPE_DO_COLLECT, &CDPSrvr::OnDoCollect );
	...
	ON_MSG(PACKETTYPE_SCROLLTASKBAR, &CDPSrvr::OnScrollTaskBar);
#endif
...
#ifdef __NEW_TASKBAR_V19
void CDPSrvr::OnScrollTaskBar(CAr &ar, DPID dpidCache, DPID dpidUser, LPBYTE, u_long)
{
	...
	for(int i = 0; i < 10; ++i)
	{
		SHORTCUT scTemp = pUser->m_playTaskBar.m_aSlotItem[0][i];
		pUser->m_playTaskBar.m_aSlotItem[0][i] = pUser->m_playTaskBar.m_aSlotItem[1][i];
		pUser->m_playTaskBar.m_aSlotItem[1][i] = pUser->m_playTaskBar.m_aSlotItem[2][i];
		pUser->m_playTaskBar.m_aSlotItem[2][i] = scTemp;
	}
