...
{ 
	...
	{
#if __VER >= 19
		p2DRender->TextOut( pt.x + 10, pt.y + 10, prj.GetText( TID_GAME_EMPTY_CURRENT_QUEST ), D3DCOLOR_ARGB( 255, 255, 255, 255 ) );
#else // __VER >= 19
...
#if __VER >= 15 // __IMPROVE_QUEST_INTERFACE
#if __VER >= 19
		DWORD dwColor = ( i == m_nSelectKeyButton ) ? D3DCOLOR_ARGB( 255, 246, 204, 77 ) : 0xffffffff;
#else // __VER >= 19
		...
		else
		{
			...
			else
			{
				...
				if( IsDBCSLeadByte( editString[ i ] ) )
				{
					m_aContextMark[ nPos ].Add( nMarkTemp );
#if __VER >= 19
					pEditString->AddString( editString[ ++i ], 0xffffffff, dwStyle );
#else // __VER >= 19
	...
	if( m_nCurArray == m_strArray.GetSize() - 1 )
	{
		...
		for( int i = 0; i < m_nWordButtonNum; i++ )
		{
...
#if __VER >= 19
			CRect Trect = CRect( x, lpWndCtrl->rect.bottom - 25, x + 70, lpWndCtrl->rect.bottom - 1 );
#endif // __VER >= 19
			...
			if( strcmp( pWordButton->szWord, "__YES__" ) == 0 )
			{
				...
				m_apWndAnswer[ j ]->Create( prj.GetText( TID_GAME_YES ) , WBS_CHILD, Trect, this, nWndId );
			...
			if( strcmp( pWordButton->szWord, "__NO__" ) == 0 )
			{
				...
				strTexture = "ButtNormal00.tga";
				m_apWndAnswer[ j ] = new CWndAnswer;
				m_apWndAnswer[ j ]->Create( prj.GetText( TID_GAME_NO ), WBS_CHILD, Trect, this, nWndId );
				m_apWndAnswer[j]->SetTexture(D3DDEVICE, MakePath( DIR_THEME, strTexture), 1);
				m_apWndAnswer[ j ]->FitTextureSize();
				m_apWndAnswer[ j ]->m_pWordButton = (WORDBUTTON*)	pWordButton;
				j++;
			...
			if( strcmp( pWordButton->szWord, "__CANCEL__" ) == 0 )
			{
				...
				strTexture = "ButtNormal00.tga";
				m_apWndAnswer[ j ] = new CWndAnswer;
				m_apWndAnswer[ j ]->Create( prj.GetText( TID_MMI_UIBUTTON_BUTTON03 ), WBS_CHILD, Trect, this, nWndId );
				m_apWndAnswer[j]->SetTexture(D3DDEVICE, MakePath( DIR_THEME, strTexture), 1);
				m_apWndAnswer[ j ]->FitTextureSize();
				m_apWndAnswer[ j ]->m_pWordButton = (WORDBUTTON*)	pWordButton;
				j++;
			...
			if( strcmp( pWordButton->szWord, "__OK__" ) == 0 )
			{
				...
				strTexture = "ButtNormal00.tga";
				m_apWndAnswer[ j ] = new CWndAnswer;
				m_apWndAnswer[ j ]->Create( prj.GetText( TID_MMI_UIBUTTON_BUTTON02 ), WBS_CHILD, Trect, this, nWndId );
				m_apWndAnswer[j]->SetTexture(D3DDEVICE, MakePath( DIR_THEME, strTexture), 1);
				m_apWndAnswer[ j ]->FitTextureSize();
				m_apWndAnswer[ j ]->m_pWordButton = (WORDBUTTON*)	pWordButton;
				j++;
