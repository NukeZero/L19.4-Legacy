...
HRESULT C2DRender::InitDeviceObjects( LPDIRECT3DDEVICE9 pd3dDevice )
{
...
#if __VER >= 19
	LPD3DXBUFFER Errors;
	if(FAILED(D3DXCreateEffectFromFile( m_pd3dDevice, "Theme\\MinimapShader.fxc", NULL, NULL, NULL, NULL, &m_pMask, &Errors)))
	{
		Error( "MinimapShader.fxc not found !" );
		if( Errors )
		{
			MessageBoxA(0, (char*)Errors->GetBufferPointer(), "DirectX 11 Compile Error", MB_OK);
		}
	}
	if( FAILED(m_pd3dDevice->CreateTexture( 180, 176, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &m_pMinimapTexture, NULL )) )
	{
		if(Errors)
		{
			MessageBoxA(0, (char*)Errors->GetBufferPointer(), "DirectX 11 Compile Error", MB_OK);
	...
	if(FAILED(m_pMinimapTexture->GetSurfaceLevel(0, &m_pMinimapSurface)) )
	{
		if(Errors)
		{
			MessageBoxA(0, (char*)Errors->GetBufferPointer(), "DirectX 11 Compile Error", MB_OK);
...
#if __VER >= 19
					RenderFillRect( CRect( x - m_ptOrigin.x, y - m_ptOrigin.y, x + size.cx - m_ptOrigin.x, y + size.cy - m_ptOrigin.y ), 0xffffffff );
#else // __VER >= 19
...
#if __VER >= 19
void C2DRender::RenderMiniMap(CTexture *pTexture)
{
	CPoint pt(0,0);
	m_pd3dDevice->SetRenderTarget(0, m_pBackBuffer);

	m_pMask->SetTexture( "MinimapTexture", m_pMinimapTexture );
	m_pMask->SetTexture( "MinimapTextureMask", pTexture->m_pTexture );
	m_pMask->SetFloat( "Blending", 0.0f );

	pt += m_ptOrigin;
	CPoint ptCenter = pTexture->m_ptCenter;
	ptCenter.x	= (LONG)( ptCenter.x );
	ptCenter.y	= (LONG)( ptCenter.y );
	pt -= ptCenter;


	FLOAT left   = (FLOAT)( m_ptOrigin.x );
	FLOAT top    = (FLOAT)( m_ptOrigin.y );
	FLOAT right  = (FLOAT)pt.x + (180);
	FLOAT bottom = (FLOAT)pt.y + (176);
	...
	m_pMask->SetTechnique(m_pMask->GetTechniqueByName("RenderFlyffMinimap"));
...
LPDIRECT3DTEXTURE9 C2DRender::RenderMergeTextures( /*CPoint pt, */CTexture *pTop, CTexture *pBottom, int nWidth, int nHeight, float fPercentageTop )
{
	...
	m_pd3dDevice->SetRenderTarget(0, m_pBackBuffer);
	//RenderTextureNative(pt, pOut, nHeight, nWidth, TRUE);
...
void C2DRender::RenderForMask_Main( CTexture *pTexture, CPoint point, DWORD m_dwAlpha, FLOAT fScaleX, FLOAT fScaleY )
{
	if( !pTexture )
		return;
	...
	FLOAT left   = (FLOAT)( pt.x );
	FLOAT top    = (FLOAT)( pt.y );
	FLOAT right  = (FLOAT)pt.x + ( fScaleX * (FLOAT)pTexture->m_size.cx );
	FLOAT bottom = (FLOAT)pt.y + ( fScaleY * (FLOAT)pTexture->m_size.cy );
	...
	pd3dDevice->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, vertex, sizeof(TEXTUREVERTEX));
	pd3dDevice->SetTexture(0, NULL);
};

void C2DRender::RenderForMask_BG( CTexture *pTexture, CPoint point, DWORD m_dwAlpha )
{
	if( !pTexture )
		return;
	...
	FLOAT left   = (FLOAT)( pt.x );
	FLOAT top    = (FLOAT)( pt.y );
	FLOAT right  = (FLOAT)pt.x + ((FLOAT)pTexture->m_size.cx );
	FLOAT bottom = (FLOAT)pt.y + ((FLOAT)pTexture->m_size.cy );
	...
	pd3dDevice->SetRenderState( D3DRS_CULLMODE, D3DCULL_NONE);
	pd3dDevice->SetRenderState( D3DRS_ALPHABLENDENABLE, TRUE);
	pd3dDevice->SetRenderState( D3DRS_TEXTUREFACTOR, D3DCOLOR_ARGB( m_dwAlpha, 0, 0, 0 ) );
...
void C2DRender::RenderTextureRotateALPHA(CPoint pt, CTexture *pTextureBG, CTexture *pTexture, FLOAT fRadian, FLOAT fBlendFactor, FLOAT fScaleX, FLOAT fScaleY )
{
	if( !pTextureBG || !pTexture )
		return;

	m_pMask->SetTexture( "MinimapTexture", pTexture->m_pTexture );
	m_pMask->SetTexture( "MinimapTextureMask", pTextureBG->m_pTexture );
	m_pMask->SetFloat( "Blending", fBlendFactor );
	...
	FLOAT left   = (FLOAT)( pt.x );
	FLOAT top    = (FLOAT)( pt.y );
	FLOAT right  = pt.x + (pTexture->m_size.cx * fScaleX);
	FLOAT bottom = pt.y + (pTexture->m_size.cy * fScaleY);
...
BOOL C2DRender::RenderTextureALPHA( CPoint pt, CTexture *pTextureBG, CTexture *pTexture, FLOAT fIllumination, FLOAT fScaleX, FLOAT fScaleY )
{
	if( !pTextureBG || !pTexture )
		return FALSE;
	...
	m_pMask->SetTexture( "MinimapTexture", pTexture->m_pTexture );
	m_pMask->SetTexture( "MinimapTextureMask", pTextureBG->m_pTexture );
	m_pMask->SetFloat( "Blending", fIllumination );
...
#if __VER >= 19
void CTexture::SetTexture( LPDIRECT3DTEXTURE9 pTexture, int nHeight, int nWidth )
{
	m_pTexture = pTexture;
	m_size = CSize(nWidth, nHeight);
	m_fuLT = 0.0f; 
...
#if __VER >= 19
void CTexturePack::RenderForMask( LPDIRECT3DDEVICE9 pd3dDevice, TEXTUREVERTEX* pVertices, int nVertexNum )
{
	if( nVertexNum == 0 )
		return;
