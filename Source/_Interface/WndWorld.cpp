...
#if __VER >= 19
		p2DRender->RenderFillRect( crBoard, D3DCOLOR_ARGB( 30, 255, 255, 255 ) );
#else // __VER >= 19
...
//	FOR_LAND( &g_World, pLand, g_World.m_nVisibilityLand, FALSE )
	FOR_LAND( g_WorldMng.Get(), pLand, g_WorldMng.Get()->m_nVisibilityLand, FALSE )
	{
		FOR_OBJ( pLand, pObj, OT_MOVER )
		{
			...
			size.cx += 5;
#if __VER >= 19
			g_Neuz.m_2DRender.TextOut( rect.left - size.cx + 1, rect.bottom - y + 1 - nFontMid, pMover->GetName(), 0xffffffff );
#else // __VER >= 19
