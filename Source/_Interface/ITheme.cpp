...
BOOL CTheme::LoadTheme( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR lpszFileName)
{
	...
	pFont->m_dwColor   = D3DCOLOR_ARGB( 255, 255, 221, 102 );
	pFont->m_dwBgColor = D3DCOLOR_ARGB( 255,   0,   0,  0 );
	m_mapFont.SetAt( _T( "NewTitle1"), pFont );

    pFont = new CD3DFont( _T("Verdana"), 9 );
	pFont->m_nOutLine  = 1;
	pFont->m_dwColor   = D3DCOLOR_ARGB( 255, 255, 221, 102 );
	pFont->m_dwBgColor = D3DCOLOR_ARGB( 255,   0,   0,  0 );
	m_mapFont.SetAt( _T( "NewTitle2"), pFont );
  ...
  */
	...
	m_mapFont.Lookup( _T("NewTitle1"), (void*&)m_pFontWndNewTitle1 );
	m_mapFont.Lookup( _T("NewTitle2"), (void*&)m_pFontWndNewTitle2 );
	m_mapFont.Lookup( _T("NewTitle1"), (void*&)m_pFontWndTitle );
	m_mapFont.Lookup( _T("Verdana"), (void*&)m_pFontChat );
	m_mapFont.Lookup( _T("Verdana"), (void*&)m_pFontText ); //Trebuchet MS
...
#if __VER >= 19
	DWORD dwColor1 = D3DCOLOR_ARGB( 255, 116, 107, 85 );
...
#if __VER >= 19
	DWORD dwColor1 = D3DCOLOR_ARGB( 255, 0, 0, 0 );
	DWORD dwColor2 = D3DCOLOR_ARGB( 255, 0, 0, 0 );
#else // __VER >= 19
...
void CTheme::RenderWndTextFrame( C2DRender* p2DRender, CRect* pRect  )
{
	...
	DWORD dwColor2 = D3DCOLOR_ARGB( 255, 255,255,255 );
#else // __VER >= 19
...
#endif // __VER >= 19
{
	...
	PlusRect.right = rect.right;
	PlusRect.bottom = rect.bottom;

	int nBreite = (PlusRect.right - PlusRect.left) / 16;
	nBreite -= 2;
	int nHahe = (PlusRect.bottom - PlusRect.top) / 16;

	for(int a = 0; a < nHahe; a++)
	{
		if( a == 0 )
			m_apTextureToolTip[ 0 ].Render(p2DRender, CPoint( PlusRect.left, PlusRect.top) );
		else if( a != 0 && (a+1) < nHahe )
			m_apTextureToolTip[ 3 ].Render(p2DRender, CPoint( PlusRect.left, PlusRect.top + (a*16)) );
		else
			m_apTextureToolTip[ 6 ].Render(p2DRender, CPoint( PlusRect.left, PlusRect.top + (a*16)) );

		for(int i = 0; i < nBreite; i++)
		{
			if( a == 0 )
				m_apTextureToolTip[ 1 ].Render(p2DRender, CPoint( PlusRect.left +((i+1) * 16), PlusRect.top) );
			else if( a != 0 && (a+1) < nHahe )
				m_apTextureToolTip[ 4 ].Render(p2DRender, CPoint( PlusRect.left +((i+1) * 16), PlusRect.top + (a*16)) );
			else
				m_apTextureToolTip[ 7 ].Render(p2DRender, CPoint( PlusRect.left +((i+1) * 16), PlusRect.top + ((nHahe-1)*16)) );
		}

		if( a == 0 )
			m_apTextureToolTip[ 2 ].Render(p2DRender, CPoint( PlusRect.left + ((nBreite+1) * 16), PlusRect.top) );
		else if( a != 0 && (a+1) < nHahe )
			m_apTextureToolTip[ 5 ].Render(p2DRender, CPoint( PlusRect.left + ((nBreite+1) * 16), PlusRect.top + (a*16)) );
		else
			m_apTextureToolTip[ 8 ].Render(p2DRender, CPoint( PlusRect.left + ((nBreite+1) * 16), PlusRect.top + (a*16)) );
	}
#else // __VER >= 19
