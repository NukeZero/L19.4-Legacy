...
#if defined( __CLIENT) && defined(__NEW_TASKBAR_V19)
...
void COption::Init()
{
	...
	m_nWidthSlideBar = 51;
	m_nTBDisplay = 0;
	CWndTaskBarInterface::m_bLock = false;
#endif
}
...
#endif
#if defined( __CLIENT) && defined(__NEW_TASKBAR_V19)
		else if(scan.Token == "TaskbarSlide" )
		{
			m_nWidthSlideBar = scan.GetNumber();
		}
		else if( scan.Token == "TaskbarDisplay" )
		{
			m_nTBDisplay = scan.GetNumber();
		}
		else if( scan.Token == "TaskbarLock" )
		{
...
#endif
#if defined( __CLIENT) && defined(__NEW_TASKBAR_V19)
	_ftprintf(fp, "TaskbarSlide %d\n", m_nWidthSlideBar);
	_ftprintf(fp, "TaskbarDisplay %d\n", m_nTBDisplay);
	BOOL bLock = CWndTaskBarInterface::m_bLock;
	_ftprintf(fp, "TaskbarLock %d\n", bLock);
#endif
	fclose(fp);
	return 1;
}
