	...
	if( IsValidObj( (CObj*)pMover ) )
	{
		...
		if( bSuccess )
		{
			...
			if (!(pMover->IsSMMode(SM_ACTPOINT)))
			{
				if (pMover->IsActiveMover())
				{
					m_SkillSL->m_USEFireALPHA = 120;
					m_SkillSL->useskill = TRUE;
						if( nUseType == SUT_QUEUESTART )
						{
							if ( clock() > m_SkillSL->m_tmCoolDown )
					{
						m_SkillSL->m_tmCoolDown = clock() + SEC(12);
...
#ifdef __NEW_TASKBAR_V19
void CDPClient::SendScrollTaskBar()
{
	SendHdr( PACKETTYPE_SCROLLTASKBAR );
}
#endif
