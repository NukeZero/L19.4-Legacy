	...
	for( int i = pt.y; i < (int)( m_pItemContainer.size() ); i++ )
	{
		...
		if( pItemBase )
		{
			...
			if( pItemElem->GetProp()->dwPackMax > 1 )
			{
				...
				p2DRender->TextOut( x * 32 + 32 - size.cx + 17, y * 32 + 32 - size.cy + 10, szTemp, 0xFF000000 );
				p2DRender->TextOut( x * 32 + 31 - size.cx + 17, y * 32 + 31 - size.cy + 10, szTemp, 0xFF85FF8A );
				
				p2DRender->SetFont( pOldFont );
#else // __VER >= 19
				p2DRender->TextOut( x * 32 + 32 - size.cx + 17, y * 32 + 32 - size.cy + 10, szTemp, 0xff0000ff );
				p2DRender->TextOut( x * 32 + 31 - size.cx + 17, y * 32 + 31 - size.cy + 10, szTemp, 0xffb0b0f0 );
#endif // __VER >= 19
			}
		}
	}
