...
*/
#if __VER >= 19
	p2DRender->TextOut( nX + 50, nY + 3, strItem, 0xffffffff );
#else // __VER >= 19
...
#endif //__S8_VENDOR_REVISION
	{
		...
		if( pItemBase )
		{
			...
			if( pItemElem->GetProp()->dwPackMax > 1 )
			{
				...
				p2DRender->TextOut( nX - 7 + 32 - size.cx+5, nY - 11 + 32 - size.cy+12, szTemp, 0xFF000000 );
				p2DRender->TextOut( nX - 7 + 31 - size.cx+5, nY - 11 + 31 - size.cy+12, szTemp, 0xFF85FF8A );
						
				p2DRender->SetFont( pOldFont );
#else // __VER >= 19
				p2DRender->TextOut( nX - 7 + 32 - size.cx+5, nY - 11 + 32 - size.cy+12, szTemp, 0xff0000ff );
				p2DRender->TextOut( nX - 7 + 31 - size.cx+5, nY - 11 + 31 - size.cy+12, szTemp, 0xffb0b0f0 );
#endif // __VER >= 19
			}
		}
	}
	...
	for( int i = 0; i < MAX_VENDITEM; i++ )
	{
		...
		if( pItemBase )
		{
			...
			if( pItemElem->GetProp()->dwPackMax > 1 )
			{
				...
				p2DRender->TextOut( x * 32 + 32 - size.cx+5, y * 32 + 32 - size.cy+12, szTemp, 0xFF000000 );
				p2DRender->TextOut( x * 32 + 31 - size.cx+5, y * 32 + 31 - size.cy+12, szTemp, 0xFF85FF8A );
						
				p2DRender->SetFont( pOldFont );
#else // __VER >= 19
				p2DRender->TextOut( x * 32 + 32 - size.cx+5, y * 32 + 32 - size.cy+12, szTemp, 0xff0000ff );
				p2DRender->TextOut( x * 32 + 31 - size.cx+5, y * 32 + 31 - size.cy+12, szTemp, 0xffb0b0f0 );
#endif // __VER >= 19
			}
		}
	}
