...
{ 
    ...
    for ( int y = 0 ; y < 9; y++ ) 
	{
		...
		wsprintf(buf, "%d:", y + 1);
#if __VER >= 19
		p2DRender->TextOut( 2, 2 + y * nMaxHeight, buf, 0xffffffff );//, lstrlen( buf ) );
#else // __VER >= 19
...
{
#if __VER >= 19
	DWORD dwColor1 = D3DCOLOR_ARGB( CWndBase::m_nAlpha - 32, 0, 0, 0 );
	DWORD dwColor2 = D3DCOLOR_ARGB( CWndBase::m_nAlpha - 32, 0, 0, 0 );
#else // __VER >= 19
...
#if __VER >= 19
		p2DRender->TextOut(2+m_maxWidth*i, 2, text, 0xFFFFFFFF );
#else // __VER >= 19
...
void CReadingList::OnDraw( C2DRender* p2DRender )
{
	...
	p2DRender->RenderFillRect(CRect(rtWindow.left+1, rtWindow.top+1, rtWindow.right-1, rtWindow.bottom-1), 0xFFFFFFFF);
#else // __VER >= 19
	p2DRender->RenderFillRect(CRect(rtWindow.left+1, rtWindow.top+1, rtWindow.right-1, rtWindow.bottom-1), 0xFF000000);
#endif // __VER >= 19
...
#if __VER >= 19
			p2DRender->TextOut(x + (16-size.cx)/2, y + (16-size.cy)/2, text, 0xFFFFFFFF );
#else // __VER >= 19
	...
	{
#if __VER >= 19
		p2DRender->TextOut( 2, 1, &*m_reading.begin(), 0xFFFFFFFF );
#else // __VER >= 19
...
#if __VER >= 19
				p2DRender->RenderFillRect( CRect( pt1.x, pt1.y, pt1.x + size.cx, pt1.y + size.cy ), D3DCOLOR_ARGB( 255, 255, 255, 255 ) );
				p2DRender->TextOut( m_ptCaret.x, m_ptCaret.y, comp, D3DCOLOR_ARGB( 255, 0, 0, 0 ) );
#else // __VER >= 19
				p2DRender->RenderFillRect( CRect( pt1.x, pt1.y, pt1.x + size.cx, pt1.y + size.cy ), D3DCOLOR_ARGB( 255, 0, 0, 0 ) );
				p2DRender->TextOut( m_ptCaret.x, m_ptCaret.y, comp, D3DCOLOR_ARGB( 255, 255, 255, 255 ) );
#endif // __VER >= 19
			}
			else
			{
#if __VER >= 19
				p2DRender->RenderFillRect( CRect( pt1.x, pt1.y, pt1.x + 2, pt1.y + nFontBottom ), D3DCOLOR_ARGB( 255, 255, 255, 255 ) );
#else // __VER >= 19
				p2DRender->RenderFillRect( CRect( pt1.x, pt1.y, pt1.x + 2, pt1.y + nFontBottom ), D3DCOLOR_ARGB( 255, 0, 0, 0 ) );
#endif // __VER >= 19
			}
		}
...
#if __VER >= 19
	DWORD dwInputColor = (PRIMARYLANGID(g_imeMgr.m_hkl)==LANG_KOREAN) ? 0xffffffff : 0xff00ffff;
#else // __VER >= 19
