...
//#endif
	...
	else if( m_bPickup_SLIDE == TRUE )
	{
		if( m_nWinSize == 0 )
		{
                ...
                rectWnd.right = RectpWndTB.left + 38;
                if(rectWnd.left < (RectpWndTB.left - 400))
                   rectWnd.left = RectpWndTB.left - 400;
                if((rectWnd.left + 13) > RectpWndTB.left)
                   rectWnd.left = RectpWndTB.left - 13;

				SetWndRect( rectWnd, TRUE );
				g_Option.m_nWidthSlideBar = rectWnd.Width();
		}
	}
#endif // __NEW_TASKBAR_V19
	if( IsWndRoot() )
