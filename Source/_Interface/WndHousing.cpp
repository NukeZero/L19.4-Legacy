...
{ 
	if(m_vecItem.size())
	{
		...
		for(iter = m_mapItem.begin(); iter != m_mapItem.end(); ++iter)
		{
			...
			{
#if __VER >= 19
				dwColor = D3DCOLOR_ARGB( 255, 0, 255, 255 );
#else // __VER >= 19
...
#if __VER >= 19
				p2DRender->RenderFillRect( renderBoxRect, 0xFF022202 );
				p2DRender->RenderRect( renderBoxRect, 0xFF077707 );
#else // __VER >= 19
	...
	for( int i = 0; i < GH_MAX_VIEW_CAPACITY; ++i )
	{
		...
		if( top < iMaxSize )
		{
...
#if __VER >= 19
				pBtn->SetTexture(D3DDEVICE, MakePath( DIR_THEME, "Buttlockerinstall.tga"), TRUE);
#else // __VER >= 19
			...
			else									
			{
				...
				//sunTODO: 확정 이미지 나오면 교체요망
#if __VER >= 19
				pBtn->SetTexture(D3DDEVICE, MakePath( DIR_THEME, "Buttlockerdismantle.tga"), TRUE);
#else // __VER >= 19
...
#if __VER >= 19
 	CTexture* pTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ITEM, pProp->szIcon), 0xffffffff );
#else // __VER >= 19
 	CTexture* pTexture = CWndBase::m_textureMng.AddTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ITEM, pProp->szIcon), 0xffff00ff );
#endif // __VER >= 19
 if( pTexture )
 	...
 	// 이름
#if __VER >= 19
 	p2DRender->TextOut( pCustom->rect.left + 50, pCustom->rect.top + 12, m_kItem.m_strName, 0xffffffff );
#else // __VER >= 19
...
#if __VER >= 19
	p2DRender->TextOut( pCustom->rect.left + 200, pCustom->rect.top + 12, strTime, 0xffffffff );
#else // __VER >= 19
