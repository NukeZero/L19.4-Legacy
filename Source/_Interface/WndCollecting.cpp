...
BOOL CCollectingWnd::SetButtonCaption(bool bIsStart)
{
	...
	{
#if __VER >= 19
		pWndButton->SetTexture(m_pParentWnd->m_pApp->m_pd3dDevice, MakePath( DIR_THEME, _T("ButtNormal01.tga")), TRUE);
		pWndButton->SetTitle( prj.GetText( TID_MMI_BUTTONSTOP ));
#else // __VER >= 19
	...
	else
	{
		...
		{
#if __VER >= 19
			pWndButton->SetTexture(g_Neuz.m_pd3dDevice, MakePath( DIR_THEME, _T("ButtNormal01.tga")), TRUE);
			pWndButton->SetTitle( prj.GetText( TID_MMI_BUTTONSTART ));
#else // __VER >= 19
