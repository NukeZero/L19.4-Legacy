...
/*	if( ::GetLanguage() != LANG_KOR )
	{
		...
		pAbout->EnableWindow(TRUE);
		pAbout->SetVisible(TRUE);	
		CWndButton* pCredit   = (CWndButton*)GetDlgItem( WIDC_CREDIT );
		pCredit->EnableWindow(TRUE);
		pCredit->SetVisible(TRUE);	
		CWndButton* pPrologue   = (CWndButton*)GetDlgItem( WIDC_PROLOGUE );
		pPrologue->EnableWindow(TRUE);
		pPrologue->SetVisible(TRUE);
		
		/*
		pOk->Move( 40, 105 );
		pPrologue->Move( 40, 135 );
		pCredit->Move( 40, 135 );	
		pAbout->Move( 40, 165 );
		pQuit->Move( 40, 195 );
		*/
		...
		SetWndRect( CRect( HanrectWindow.left, HanrectWindow.top, HanrectWindow.right, HanrectWindow.bottom + 65 ) );
...
#if __VER >= 19
	pWndListMulti->m_nFontColor = 0xffffffff;
	pWndListMulti->m_nSelectColor = 0xff27C4CB;
#else // __VER >= 19
...
#if __VER >= 19
	pWndList->m_nFontColor = 0xffffffff;
	pWndList->m_nSelectColor = 0xff27C4CB;
#else // __VER >= 19
	...
	for( int i = 0; i < MAX_CHARACTER_LIST; i++ )
	{
		...
		if( g_Neuz.m_apPlayer[i] != NULL )
		{
			...
			if( m_nSelectCharacter == i )
			{
                ...
                switch( i )
                {
                case 0: pWndStaticChar[1]->m_dwColor = 0xFFF6CC4D, pWndStaticChar[2]->m_dwColor = 0xFF666666, pWndStaticChar[3]->m_dwColor = 0xFF666666; break;
                case 1: pWndStaticChar[1]->m_dwColor = 0xFF666666, pWndStaticChar[2]->m_dwColor = 0xFFF6CC4D, pWndStaticChar[3]->m_dwColor = 0xFF666666; break;
                case 2: pWndStaticChar[1]->m_dwColor = 0xFF666666, pWndStaticChar[2]->m_dwColor = 0xFF666666, pWndStaticChar[3]->m_dwColor = 0xFFF6CC4D; break;
                }
#else
				CRect rectTemp = rect;
...
//				pWndDelete->EnableWindow( TRUE );
			...
			else
			{
				...
				switch( i )
				{
				case 0: pWndStaticChar[1]->m_dwColor = 0xFF09BAB6, pWndStaticChar[2]->m_dwColor = 0xFF666666, pWndStaticChar[3]->m_dwColor = 0xFF666666; break;
				case 1: pWndStaticChar[1]->m_dwColor = 0xFF666666, pWndStaticChar[2]->m_dwColor = 0xFF09BAB6, pWndStaticChar[3]->m_dwColor = 0xFF666666; break;
				case 2: pWndStaticChar[1]->m_dwColor = 0xFF666666, pWndStaticChar[2]->m_dwColor = 0xFF666666, pWndStaticChar[3]->m_dwColor = 0xFF09BAB6; break;
				}
#endif // __VER >= 17
			}
		}
