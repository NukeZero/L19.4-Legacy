	...
	if( lpQuest || bComplete ) 
	{
		...
		if( bComplete )
		{
			for( int i = QS_END; i >= 0; i-- )
			{
				...
				{
#if __VER >= 19
					pTextDesc->SetString( pQuestState->m_szDesc, 0xffffffff );
#else // __VER >= 19
...
void MakeQuestConditionItems( DWORD dwQuestID, CWndTreeCtrl* pWndTreeCtrl, BOOL bClick, LPTREEELEM lpTreeElem, DWORD dwStartColor, DWORD dwEndColor, DWORD dwSelectColor )
{
	...
	if( lpQuest->m_nState == ( i - 1 ) || lpQuest->m_nState == 14 )
	{
		...
		if( pQuestProp->m_szEndCondCharacter[ 0 ] )
		{
			...
			if (pQuestProp->m_MeetCharacterGoalData.mustRequest)
			{
				g_DPlay.SendReqNPCPos(dwQuestID); // questId
				pQuestProp->m_MeetCharacterGoalData.mustRequest = false;
			}
#endif
#if __VER >= 13 // __QUEST_HELPER
...
DWORD SetQuestDestinationInformation( DWORD dwQuestID, DWORD dwGoalIndex )
{
	...
	for( i = 0; i < nEndCondItemNum; ++i )
	{
		...
		if (roQuestGoalData.m_dwGoalIndex == 0)
			continue;
		if (roQuestGoalData.m_dwGoalIndex == dwGoalIndex)
		{
			if (roQuestGoalData.worldId == g_pPlayer->GetWorld()->GetID())
			{
				Error("same map!");
			...
			{
				Error("fake map! %d", roQuestGoalData.worldId);
			}
	...
	for( i = 0 ; i < MAX_QUEST_COND_KILL; ++i )
	{
#ifdef __KIA_GOALDATA_FIX
