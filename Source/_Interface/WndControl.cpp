...
#if __VER >= 16
BOOL CWndStatic::Create( LPCTSTR lpszCaption, DWORD dwStyle, const RECT& rect, CWndBase* pParentWnd, UINT nID, int nRed, int nGreen, int nBlue )
#else // __VER >= 16
...
#if __VER >= 16
			m_dwColor = D3DCOLOR_ARGB( 255, nRed, nGreen, nBlue );
#else // __VER >= 16
...
void CWndStatic::PaintFrame( C2DRender* p2DRender )
{
	...
	CRect rect = GetWindowRect();
#if __VER >= 19
	p2DRender->RenderRect( rect, 0xffffffff );
#else // __VER >= 19
...
*/	
	...
	if(m_dwStyle & WBS_MENUITEM)
	{
...
#if __VER >= 19
			p2DRender->TextOut( pt.x + 4 + 20, pt.y + 4, m_strTitle, 0xffffffff  );
#else // __VER >= 19
		...
		else
		{
			...
			{
#if __VER >= 19
				p2DRender->RenderLine( CPoint( 2,  2), CPoint( 5, 8), 0xffffffff );
				p2DRender->RenderLine( CPoint( 2,  2), CPoint( 6, 8), 0xffffffff );
				p2DRender->RenderLine( CPoint( 5,  8), CPoint( 8, 3), 0xffffffff );
#else // __VER >= 19
...
#if __VER >= 19
				p2DRender->TextOut( m_rectClient.right - 32, pt.y + 4, strHotkey, 0xffffffff );
#else // __VER >= 19
...
void CWndListBox::OnDraw(C2DRender* p2DRender)
{
	...
	if( pWnd->GetWndId() == APP_DIALOG_EVENT )
	{
		...
		for( unsigned i = nPos; i < nEnd; ++i )
		{
			...
			if( i == m_nCurSelect )
			{
				CRect DrawRect = CRect(0, pt.y+2, 434, pt.y+35 );
			...
			p2DRender->RenderLine(CPoint(0,pt.y),CPoint(GetWindowRect().Width()-26,pt.y),0xFF776655);
		}
...
#ifdef __ITEMLINK
void CWndText::BlockSetItemLink( DWORD dwUserId, DWORD dwObjId )
{
	if( m_bEnableClipboard )
	{
		DWORD dwBlockBegin;
		...
		{
			m_string.SetItemLink( dwBlockBegin, dwBlockEnd - dwBlockBegin, dwUserId, dwObjId );
		}
	}	
}
#endif
...
#ifdef __ITEMLINK
void CWndText::SetString( LPCTSTR pszString, DWORD dwColor, DWORD dwPlayerId, DWORD dwObjId )
#else
...
#if __VER >= 15
	m_string.SetParsingString( pszString, dwColor, 0x00000000, 0, 0x00000001, FALSE, dwPlayerId, dwObjId );
#else
	m_string.SetParsingString( pszString, dwColor, 0x00000000, 0, 0x00000001, dwPlayerId, dwObjId );
#endif
#else
...
#ifdef __ITEMLINK
void CWndText::AddString(LPCTSTR pszString, DWORD dwColor, DWORD dwPStyle, DWORD dwPlayerId, DWORD dwObjId )
#else
...
#ifdef __ITEMLINK
	m_string.AddParsingString( pszString, dwColor, 0x00000000, 0, dwPStyle, dwPlayerId, dwObjId );//+= pszString;
#else
...
#if __VER >= 19
	if(m_nLargeWidth < 130)
		m_nLargeWidth = 130;
	
	pWndButton->Create(lpszNewItem, WBS_MENUITEM | WBS_HIGHLIGHT/* | WBS_NOMENUICON*/, CRect( 10, 16 + ( nCount * 22 ), m_nLargeWidth - 10, 18 + ( nCount * 22 ) + 20 ), this, nIDNewItem );
#else // __VER >= 19
...
#if __VER >= 19
		rect.right = rect.left + m_nLargeWidth - 12;
#else // __VER >= 19
...
#if __VER >= 19
	SetWndRect( CRect( -10, -6, m_nLargeWidth, 32 + ( ( nCount + 1 ) * 22 ) ) );
#else // __VER >= 19
...
BOOL CWndMenu::OnEraseBkgnd( C2DRender* p2DRender )
{
	...
	for( int j = 0 ; j < 9 ; ++ j )
	{
		...
		if( j > 5 )
		    sprintf( szBuf, "%02d",j+3 );
		else if( j <= 5 ) 
		    sprintf( szBuf, "%02d",j+0 );
		...
		for( int i = 0; i < nTabSize; ++i )
		{
			...
			if( pItem )
			{
				...
				case ALIGN_LEFT:
					{
#if __VER >= 19
						DWORD dwTextColor = ( i != m_nCurSelect ) ? 0xff958973 : 0xffffffff;
#else // __VER >= 19
				...
				{
#if __VER >= 19
					p2DRender->TextOut( i * nLength + 2, y, pItem->pszText, 0xff958973 );
#else // __VER >= 19
