...
#ifdef __3RD_LEGEND16
	if( !pTarget->IsDie() && pTarget->GetWorld() == GetWorld() )
	{
		...
		{
			if( pSkillProp->dwID == SI_LOD_SUP_PULLING )
			{
				pTarget->SetPos( GetPos() );
			}
			else if( pSkillProp->dwID == SI_WIN_YOYO_BACKSTEP )
			{
				...
				if( GetWorld()->VecInWorld( fX, fZ ) )
				{
					...
					g_UserMng.AddSetPos( (CCtrl*)this, vPos );
					SetPos( vPos );

					if( IsPlayer() )
					{
						((CUser*)this)->Notify();
						((CUser*)this)->AddSetTarget( pTarget->GetId() );
						((CUser*)this)->m_idSetTarget = pTarget->GetId();
					}
				}
				else
					return FALSE;
...
BOOL CMover::DoUseItem( DWORD dwData, DWORD dwFocusId, int nPart  )
{
	...
	else	// 일반적인 아이템 사용 
	{
		...
		switch( pItemProp->dwItemKind2 )
		{
			...
			{
				int nResult = 0;
				if (IsNeedTarget(pItemProp))
				{
					return FALSE;
				}

				nResult = DoUseItemOnce(pItemProp, pItemElem);
				{
					if (0 < nResult)
					{
						if (nResult == 2)
						...
						{
							((CUser*)this)->AddDefinedText(TID_GAME_NOTOVERLAP_USE, "");
						}
						return FALSE;
					}
				}
			...
			case IK2_SKILL:
				...
				break;
#ifdef __TREASUREBOX
			case IK2_TREASURE: return FALSE; break;
#endif // __TREASUREBOX
			default:
				{
					if( pItemProp->dwSkillTime != NULL_ID )		// 지속시간이 있는 아이템
