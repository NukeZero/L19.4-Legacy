...
#if __VER >= 19
		p2DRender->TextOut( 190, 18, 1, 1, strCampusPoint, 0xffF6CC4D );
#else // __VER >= 19
...
#if __VER >= 19
	pWndText->AddString( strMessage, 0xFFF6CC4D );
#else // __VER >= 19
...
void CWndInstantMsg::AddPostMessage( LPCTSTR lpszSendName )
{
	...
	CWndText* pWndText = (CWndText*)GetDlgItem( WIDC_TEXT );
#if __VER >= 19
	pWndText->AddString( lpszSendName, 0xFFF6CC4D );
#else // __VER >= 19
...
void CWndMessage::AddMessage( LPCTSTR lpszFrom, LPCTSTR lpszMessage )
{
	...
	{
#if __VER >= 19
		strMessage.Format("#cff6161ff%s%s :#nc\n  %s\n", lpszFrom, prj.GetText(TID_GAME_FROM3), lpszMessage);
#else // __VER >= 19
		strMessage.Format( "#cff0000ff%s%s :#nc\n  %s\n", lpszFrom, prj.GetText(TID_GAME_FROM3), lpszMessage );
#endif // __VER >= 19
	}
