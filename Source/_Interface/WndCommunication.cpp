...
#if __VER >= 19
	m_wndMacroChat.Create( "", WBS_NODRAWFRAME, lpWndCtrl->rect, this, WIDC_MACRO_CHAT );
#else // __VER >= 19
...
BOOL CWndChat::Process ()
{
	...
	for( int i = nPos ; i < nLineCount ; ++i )
	{
		...
		{
			//CEditString str = m_wndText.m_string.GetEditString( i );
			LONG lOffset = m_wndText.GetOffset(point);
			if(lOffset != -1)
			{
				if( m_wndText.m_string.m_adwPlayerId.GetSize() > lOffset )
				{
					DWORD dwPlayerId = m_wndText.m_string.m_adwPlayerId.GetAt( lOffset );
					if( dwPlayerId != 0 )
					{
						m_lOffsetBegin = m_lOffsetEnd = lOffset;
						while( dwPlayerId == m_wndText.m_string.m_adwPlayerId.GetAt( --m_lOffsetBegin ) ){}
						m_lOffsetBegin++;
						while( dwPlayerId == m_wndText.m_string.m_adwPlayerId.GetAt( ++m_lOffsetEnd ) ){}
						m_wndText.m_string.SetStyle( m_lOffsetBegin, m_lOffsetEnd - m_lOffsetBegin, ESSTY_UNDERLINE );
						bLink = true;
	...
	if( !bLink && (m_lOffsetBegin != -1 && m_lOffsetEnd != -1) ){
		m_wndText.m_string.ClearStyle( m_lOffsetBegin, m_lOffsetEnd - m_lOffsetBegin, ESSTY_UNDERLINE );
		m_lOffsetBegin = -1;
		m_lOffsetEnd = -1;
	}
...
void CWndChat::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	for( int i = nPos ; i < nLineCount ; ++i )
	{
		if( (int)( ((i-nPos) * dwMaxHeight) ) <= point.y && point.y < (int)( (dwMaxHeight * (i-nPos + 1)) ) )
		{
			...
			//Error("Going to");
			LONG lOffset = m_wndText.GetOffset(point);
			//Error("Offset: %d", lOffset );
			if(lOffset != -1)
			{
				//Error("%i, %i",m_wndText.m_string.m_adwPlayerId.GetAt( lOffset ), m_wndText.m_string.m_adwObjId.GetAt( lOffset ));
				if( m_wndText.m_string.m_adwPlayerId.GetSize() > lOffset && m_wndText.m_string.m_adwPlayerId.GetAt( lOffset ) != 0 )
				{
					g_DPlay.SendItemLink( m_wndText.m_string.m_adwPlayerId.GetAt( lOffset ), m_wndText.m_string.m_adwObjId.GetAt( lOffset ) );
					bDid = TRUE;
					//Error("Ok2");
					break;
				}
