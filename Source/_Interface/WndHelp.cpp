...
#if __VER >= 19
	p2DRender->TextOut( rect.left + 10, rect.top + 8, m_strKeyword, 0xffffffff );
#else // __VER >= 19
...
{ 
	...
	m_nPosString = 0;
#if __VER >= 19
	pWndText->SetString( m_aString.GetAt( 0 ), 0xffffffff );
#else // __VER >= 19
	...
	case WIDC_NEXT:
		{
			...
			CWndText* pWndText = (CWndText*)GetDlgItem( WIDC_TEXT );
#if __VER >= 19
			pWndText->SetString( m_aString.GetAt( m_nPosString ), 0xffffffff );
#else // __VER >= 19
