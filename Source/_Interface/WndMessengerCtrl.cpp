...
void CWndFriendCtrlEx::OnDraw( C2DRender* p2DRender ) 
{
	...
	for(int i=0; iter<m_vPlayerList.end(); i++, iter++)
	{
		...
		{
#if __VER >= 19
			p2DRender->TextOut( LEVEL_TEXT_X, pt.y + 3, strFormat, 0xffffffff );
#else // __VER >= 19
...
#endif // __HOUSING
#if __VER >= 19
			p2DRender->TextOut( 122, pt.y + 3, strFormat, 0xffffffff );
#else // __VER >= 19
...
#if __VER >= 15 // __CAMPUS
#if __VER >= 19
		p2DRender->TextOut( 127, pt.y + 3, strFormat, 0xffffffff );
#else // __VER >= 19
...
void CWndCampus::OnDraw( C2DRender* p2DRender )
{
	...
	if( pCampus->IsMaster( g_pPlayer->m_idPlayer ) == TRUE )
	{
		...
		for( int i = 0; iter < m_vDisciplePlayer.end(); ++i, ++iter )
		{
			...
			// Draw Name
#if __VER >= 19
			DWORD dwColor = ( i == m_nCurSelectedDisciple ) ? 0xff6060ff : 0xffffffff;
#else // __VER >= 19
	...
	else if( pCampus->IsPupil( g_pPlayer->m_idPlayer ) == TRUE )
	{
		...
		// Draw Name
#if __VER >= 19
		DWORD dwColor = ( m_bCurSelectedMaster == TRUE ) ? 0xff6060ff : 0xffffffff;
#else // __VER >= 19
