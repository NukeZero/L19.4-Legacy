...
#ifdef __ITEMLINK
	m_adwPlayerId.Append( stringSrc.m_adwPlayerId );
	m_adwObjId.Append( stringSrc.m_adwObjId );
#endif
...
#ifdef __ITEMLINK
void CEditString::SetItemLink( DWORD dwPlayerId, DWORD dwObjId )
{
	m_adwPlayerId.SetSize( GetLength() );
	m_adwObjId.SetSize( GetLength() );
	for( int i = 0; i < GetLength(); i++)
	{
		m_adwPlayerId.SetAt( i, dwPlayerId );
		m_adwObjId.SetAt( i, dwObjId );
	}
}
void CEditString::SetItemLink( int nPos, int nLength, DWORD dwPlayerId, DWORD dwObjId )
{
	for( int i = nPos; i < nPos + nLength; i++)
	{
		m_adwPlayerId.SetAt( i, dwPlayerId );
...
#ifdef __ITEMLINK
	m_adwPlayerId.RemoveAll();
	m_adwObjId.RemoveAll();
#endif
...
void CEditString::GetTextFormat( CString& str )
{
	...
	if( nLen )
	{
		...
		DWORD dwStyleU;
#if __VER >= 19
		DWORD dwOldColor = 0xffffffff;
#else // __VER >= 19
		DWORD dwOldColor = 0xff000000;
#endif // __VER >= 19
		DWORD dwOldStyleS = 0;
		DWORD dwOldStyleB = 0;
		DWORD dwOldStyleU = 0;
#ifdef __ITEMLINK
		DWORD dwOldPlayerId = 0;
		DWORD dwPlayerId = 0;
		DWORD dwObjId = 0;
		int nLinkCnt = 0;
#endif
		...
		for( int i=0; i<nLen; i++ )
		{
...
#ifdef __ITEMLINK
			dwPlayerId = m_adwPlayerId.GetAt(i);
			dwObjId = m_adwObjId.GetAt(i);
#endif
			...
			if( dwPlayerId != dwOldPlayerId )
			{
				...
				{
					CString strLink;
					strLink.Format( "#i%07d%04d", dwPlayerId, dwObjId );
					str += strLink;
					nLinkCnt++;
				}
			}
...
#ifdef __ITEMLINK
void CEditString::ParsingString( LPCTSTR lpsz, DWORD dwColor, DWORD dwStyle, WORD wCodePage, CString& string, CDWordArray& adwColor, CByteArray& abyStyle, CWordArray& awCodePage, CDWordArray& adwPlayerId, CDWordArray& adwObjId, DWORD dwPStyle, DWORD dwPlayerId, DWORD dwObjId )
#else
...
#endif
{
	...
	TCHAR szColor[9];
#ifdef __ITEMLINK
	DWORD dwCurPlayerId = dwPlayerId;
	DWORD dwCurObjId = dwObjId;
	TCHAR szPlayerId[8];
	TCHAR szObjId[5];
#endif
	...
	for( int i = 0; i < nLen; i++ )
	{
		if( lpsz[ i ] == '#' ) // 인식 코드
		{
			...
			case 'i':
				...
				memcpy( szPlayerId, &lpsz[ i ], 7 );
				szPlayerId[ 7 ] = 0;
				dwCurPlayerId = atoi( szPlayerId );
				i += 7;

				memcpy( szObjId, &lpsz[ i ],  4);
				szObjId[ 4 ] = 0;
				dwCurObjId = atoi( szObjId );
				i += 3;
...
#ifdef __ITEMLINK
void CEditString::AddParsingString( LPCTSTR lpsz, DWORD dwColor, DWORD dwStyle, WORD wCodePage, DWORD dwPStyle, DWORD dwPlayerId, DWORD dwObjId )
{ 
	int nLine = GetLineCount() - 1;
	ParsingString( lpsz, dwColor, dwStyle, CODE_PAGE(wCodePage),(CString&)*this, m_adwColor, m_abyStyle, m_awCodePage, m_adwPlayerId, m_adwObjId, dwPStyle, dwPlayerId, dwObjId );
	Align( m_pFont, nLine );
}
#if __VER >= 15 // __IMPROVE_QUEST_INTERFACE
void CEditString::SetParsingString( LPCTSTR lpsz, DWORD dwColor, DWORD dwStyle, WORD wCodePage, DWORD dwPStyle, BOOL bNoInitAlign, DWORD dwPlayerId, DWORD dwObjId )
{ 
	Empty();
	ParsingString( lpsz, dwColor, dwStyle, CODE_PAGE(wCodePage), (CString&)*this, m_adwColor, m_abyStyle, m_awCodePage, m_adwPlayerId, m_adwObjId, dwPStyle,  dwPlayerId, dwObjId );
...
#else // __IMPROVE_QUEST_INTERFACE
void CEditString::SetParsingString( LPCTSTR lpsz, DWORD dwColor, DWORD dwStyle, WORD wCodePage, DWORD dwPStyle, DWORD dwPlayerId, DWORD dwObjId )
{ 
	Empty();
	ParsingString( lpsz, dwColor, dwStyle, CODE_PAGE(wCodePage), (CString&)*this, m_adwColor, m_abyStyle, m_awCodePage, m_adwPlayerId, m_adwObjId, dwPStyle,  dwPlayerId, dwObjId );
	Align( m_pFont, 0 );
...
#else // __ITEMLINK
void CEditString::AddString( LPCTSTR lpsz, DWORD dwColor, DWORD dwStyle, WORD wCodePage, DWORD dwPlayerId, DWORD dwObjId )
{ 
	int nLine = GetLineCount() - 1;
	int nBegin = GetLength();
	int nEnd = nBegin + _tcslen( lpsz );
	*((CString*)this) += lpsz; 
	for( int i = nBegin; i < nEnd; i++ )
		m_adwColor.SetAtGrow( i, dwColor ); 
	for( int i = nBegin; i < nEnd; i++ )
		m_adwPlayerId.SetAtGrow( i, dwPlayerId );
	for( int i = nBegin; i < nEnd; i++ )
		m_adwObjId.SetAtGrow( i, dwObjId ); 
	for( int i = nBegin; i < nEnd; i++ )
		m_abyStyle.SetAtGrow( i, (BYTE)( dwStyle ) ); 
	for( int i = nBegin; i < nEnd; i++ )
		m_awCodePage.SetAtGrow( i, CODE_PAGE(wCodePage) ); 
...
void CEditString::AddString( char ch, DWORD dwColor, DWORD dwStyle, WORD wCodePage, DWORD dwPlayerId, DWORD dwObjId )
{ 
	int nLine = GetLineCount() - 1;
	*((CString*)this) += ch; 
	m_adwColor.Add( dwColor ); 
	m_adwPlayerId.Add( dwPlayerId );
	m_adwObjId.Add( dwObjId ); 
	m_abyStyle.Add( (BYTE)( dwStyle ) ); 
	m_awCodePage.Add( CODE_PAGE(wCodePage) );
	Align( m_pFont, nLine );
}
...
#else // __ITEMLINK
void CEditString::SetString( LPCTSTR lpsz, DWORD dwColor, DWORD dwStyle, WORD wCodePage, DWORD dwPlayerId, DWORD dwObjId )
{ 
	Empty();
	*((CString*)this) = lpsz; 
	SetColor( dwColor );
	SetItemLink( dwPlayerId, dwObjId );
	SetStyle( dwStyle );
	SetCodePage( CODE_PAGE(wCodePage) );
	Align( m_pFont, 0 );
	//ParsingString( lpsz, dwColor );
}
void CEditString::SetString( char* lpsz, DWORD dwColor , DWORD dwStyle, WORD wCodePage, DWORD dwPlayerId, DWORD dwObjId )
{
	Empty();
	*((CString*)this) = lpsz; 
	SetColor( dwColor );
...
#ifdef __ITEMLINK
	m_adwPlayerId.Append( string.m_adwPlayerId );
	m_adwObjId.Append( string.m_adwObjId );
#endif
	Align( m_pFont, nLine );
}
...
#ifdef __ITEMLINK
int CEditString::Insert( int nIndex, TCHAR ch, DWORD dwColor, DWORD dwStyle, WORD wCodePage, DWORD dwPlayerId, DWORD dwObjId )
#else
...
#ifdef __ITEMLINK
	m_adwPlayerId.InsertAt( nIndex, dwPlayerId );
	m_adwObjId.InsertAt( nIndex, dwObjId );
#endif
...
#ifdef __ITEMLINK
int CEditString::Insert( int nIndex, LPCTSTR pstr, DWORD dwColor, DWORD dwStyle, WORD wCodePage, DWORD dwPlayerId, DWORD dwObjId )
#else
...
#ifdef __ITEMLINK
	m_adwPlayerId.InsertAt( nIndex, dwPlayerId, _tcslen( pstr) );
	m_adwObjId.InsertAt( nIndex, dwObjId, _tcslen( pstr) );
#endif
...
#ifdef __ITEMLINK
	m_adwPlayerId.RemoveAt( nIndex, nCount );
	m_adwObjId.RemoveAt( nIndex, nCount );
#endif
...
#ifdef __ITEMLINK
	m_adwPlayerId.RemoveAt( nBeginOffset, nLengthCount );
	m_adwObjId.RemoveAt( nBeginOffset, nLengthCount );
#endif
...
#ifdef __ITEMLINK
void CEditString::SetAt( int nIndex, TCHAR ch, DWORD dwColor, DWORD dwStyle, WORD wCodePage, DWORD dwPlayerId, DWORD dwObjId )
#else
...
#ifdef __ITEMLINK
	m_adwPlayerId.SetAt( nIndex, dwPlayerId );
	m_adwObjId.SetAt( nIndex, dwObjId );
#endif
...
#ifdef __ITEMLINK
			retval.m_adwPlayerId[i] = m_adwPlayerId[ begin + i ];
			retval.m_adwObjId[i] = m_adwObjId[ begin + i ];
#endif
